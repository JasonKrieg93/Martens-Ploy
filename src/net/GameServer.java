...
public class GameServer extends Thread{
	...
	private void parsePacket(byte[] data, InetAddress address, int port) {
		...
		case USE:
			packet = new Packet03Use(data);
			this.handleUse(((Packet03Use) packet));
			break;
	...
	public void addConnection(PlayerMP player, Packet00Login packet) {
		...
		for(PlayerMP p : this.connectedPlayers) {
			if(player.getUsername().equalsIgnoreCase(p.getUsername())) {
				...
				//add the new player to the currently connected player's game
				sendData(packet.getData(), p.ipAddress, p.port);
				
				//add the already connected player to the game of the newly connecting player
	...
	private void handleUse(Packet03Use packet) {
		if(getPlayerMP(packet.getUsername()) != null) {
			int index = getPlayerMPIndex(packet.getUsername());
			PlayerMP player = this.connectedPlayers.get(index);
			player.x = packet.getX();
			player.y = packet.getY();
			player.setMovingDir(packet.getMovingDir());
			player.setUsing(packet.isUsing());
			packet.writeData(this);
		}
	}
}
