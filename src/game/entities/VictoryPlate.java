...
public class VictoryPlate extends Entity {
	
	private String plateName;
	
	public VictoryPlate(Level level, int x, int y, String plateName) {
	...
	private void activatePressurePlate() {
		...
		for(Entity e : Game.game.level.getEntities()) {
			if(!e.isPressurePlate) {
				//This if condition is the only difference between a regular plate and a victoryplate (and the colour) I only want players on here, not rocks
	...
	public void render(Screen screen) {
		int xTile = 0;
		int colour = Colours.get(-1, 225, 335, -1);
		if(this.isInteracted) {
			xTile = 1;
			colour = Colours.get(-1, 114, 225, -1);
		}
		int yTile = 2;
